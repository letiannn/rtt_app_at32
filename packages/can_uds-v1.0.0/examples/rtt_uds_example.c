...
RTT_UDS_DOWNLOAD_SERVICE_DEFINE(download_service);
#endif
...
#define UDS_FAL_DEV_NAME "ota"

RTT_UDS_FAL_SERVICE_DEFINE(fal_service, UDS_FAL_DEV_NAME);
...
static int uds_example(int argc, char **argv)
{
    ...
    if (!rt_strcmp(cmd, "start"))
    {
        ...
        rtt_uds_fal_service_mount(uds_env, &fal_service);
...
#ifdef UDS_ENABLE_DOWNLOAD_SVC
        rtt_uds_download_service_mount(uds_env, &download_service);
