...
typedef struct
{
    /* --- Configuration --- */
    const char *dev_name;              
    struct fal_partition *fal_partition;
 ...
 * @param _dev_name_str RT-Thread Device Name (e.g., "uds_vcon").
 */
#define RTT_UDS_FAL_SERVICE_INIT(_svc_ptr, _name_str, _dev_name_str) \
    do                                                                   \
    {                                                                    \
        rt_memset((_svc_ptr), 0, sizeof(uds_console_service_t));         \
        (_svc_ptr)->dev_name = (_dev_name_str);                          \
...
#define RTT_UDS_FAL_SERVICE_DEFINE(_name, _dev_name)          \
    static uds_fal_service_t _name = {                        \
        .dev_name = _dev_name,                                    \
        .fal_partition = NULL,                                    \ 
        .service_node = {                                         \
            .list = RT_LIST_OBJECT_INIT(_name.service_node.list), \
...
#ifndef UDS_DOWNLOAD_MAX_PATH_LEN
#define UDS_DOWNLOAD_MAX_PATH_LEN 64
#endif

typedef enum
{
    DOWNLOAD_MODE_IDLE = 0,
    DOWNLOAD_MODE_SYNC,
} uds_download_mode_t;

/**
 * @brief Download Service Context
 * @details Stores the state of the current Download transfer session.
 *          Support instantiation to allow multiple contexts (though UDS usually runs one).
 */
typedef struct
{
    ...
    uint32_t total_size;    /**< Expected total size */
    uint32_t current_pos;   /**< Current read/write offset */
    uds_download_mode_t mode;   /**< Current transfer state */
    uint32_t current_crc;   /**< Running CRC32 */
    struct fal_partition *fal_partition;

    /* Service Nodes */
    uds_service_node_t req_node;     /* 0x34 UDS_EVT_RequestDownload */
    uds_service_node_t data_node;    /* 0x36 TransferData */
    uds_service_node_t exit_node;    /* 0x37 RequestTransferExit */
    uds_service_node_t timeout_node; /* Session Timeout Handler */
} uds_download_service_t;

/* --- Macros for Static Definition --- */

/**
 * @brief  Runtime initialization for DOWNLOAD Service Instance.
 * @details Clears the structure, resets FD, and initializes list nodes/names.
 * 
 * @param _svc_ptr  Pointer to the service struct.
 * @param _name_str Base name string (e.g., "file_svc").
 */
#define RTT_UDS_DOWNLOAD_SERVICE_INIT(_svc_ptr, _name_str)        \
    do                                                            \
    {                                                             \
        rt_memset((_svc_ptr), 0, sizeof(uds_download_service_t)); \
        (_svc_ptr)->mode = DOWNLOAD_MODE_IDLE;                    \
        rt_list_init(&(_svc_ptr)->req_node.list);                 \
        (_svc_ptr)->req_node.name = _name_str "_req";             \
        (_svc_ptr)->req_node.context = (_svc_ptr);                \
        rt_list_init(&(_svc_ptr)->data_node.list);                \
...
/**
 * @brief  Statically define a Download Service Instance.
 * @param _name Name of the variable.
 */
#define RTT_UDS_DOWNLOAD_SERVICE_DEFINE(_name)                                                                             \
    static uds_download_service_t _name = {                                                                                \
        .mode = DOWNLOAD_MODE_IDLE,                                                                                        \
        .req_node = {                                                                                                      \
            .list = RT_LIST_OBJECT_INIT(_name.req_node.list),                                                              \
            .name = #_name "_req",                                                                                         \
            .context = &_name },                                                                                           \
