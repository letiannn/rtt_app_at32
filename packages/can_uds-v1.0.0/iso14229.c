...
UDSErr_t UDSClientInit(UDSClient_t *client) {
    ...
    if (client->p2_star_ms < client->p2_ms) {
        UDS_LOGE(DBG_TAG, "p2_star_ms must be >= p2_ms");
...
static void changeState(UDSClient_t *client, uint8_t state) {
    if (state != client->state) {
        UDS_LOGI(DBG_TAG, "client state: %s (%d) -> %s (%d)", ClientStateName(client->state),
...
static UDSErr_t HandleServerResponse(UDSClient_t *client) {
    if (0x7F == client->recv_buf[0]) {
        if (UDS_NRC_RequestCorrectlyReceived_ResponsePending == client->recv_buf[2]) {
            ...
            UDS_LOGI(DBG_TAG, "got RCRRP, set p2 timer to %" PRIu32 "", client->p2_timer);
        ...
        case kSID_DIAGNOSTIC_SESSION_CONTROL: {
            if (client->recv_size < UDS_0X10_RESP_LEN) {
                UDS_LOGI(DBG_TAG, "Error: SID %x response too short",
            ...
            UDS_LOGI(DBG_TAG, "received new timings: p2: %" PRIu16 ", p2*: %" PRIu32, p2, p2_star);
            client->p2_ms = p2;
    ...
    case STATE_SENDING: {
        {
            ...
            if (len < 0) {
                UDS_LOGE(DBG_TAG, "transport returned error %zd", len);
            } else if (len == 0) {
                ; // expected
            } else {
                UDS_LOGW(DBG_TAG, "received %zd unexpected bytes:", len);
                UDS_LOG_SDU(DBG_TAG, client->recv_buf, len, &info);
            }
        }
        ...
        if (ret < 0) {
            err = UDS_ERR_TPORT;
            UDS_LOGI(DBG_TAG, "tport err: %zd", ret);
        } else if (0 == ret) {
            UDS_LOGI(DBG_TAG, "send in progress...");
    ...
    case STATE_AWAIT_RESPONSE: {
        ...
        if (len < 0) {
            ...
            if (UDSTimeAfter(UDSMillis(), client->p2_timer)) {
                UDS_LOGI(DBG_TAG, "p2 timeout");
                err = UDS_ERR_TIMEOUT;
                changeState(client, STATE_IDLE);
            }
        } else {
            UDS_LOGI(DBG_TAG, "received %zd bytes. Processing...", len);
...
UDSErr_t UDSSendRoutineCtrl(UDSClient_t *client, uint8_t type, uint16_t routineIdentifier,
    ...
    if (size) {
        ...
        if (NULL != data) {
            UDS_LOGI(DBG_TAG, "warning: size zero and data non-null");
        }
    }
...
UDSErr_t UDSSendTransferData(UDSClient_t *client, uint8_t blockSequenceCounter,
    ...
    UDS_LOGI(DBG_TAG, "size: %d, blocklength: %d", size, blockLength);
...
UDSErr_t UDSCtrlDTCSetting(UDSClient_t *client, uint8_t dtcSettingType, uint8_t *data,
    ...
    if (NULL == data) {
        ...
        if (size == 0) {
            UDS_LOGI(DBG_TAG, "warning: size == 0 and data is non-null");
        }
...
UDSErr_t UDSSendSecurityAccess(UDSClient_t *client, uint8_t level, uint8_t *data, uint16_t size) {
    ...
    if (size == 0 && NULL != data) {
        UDS_LOGE(DBG_TAG, "size == 0 and data is non-null");
        return UDS_ERR_INVALID_ARG;
    }
    if (size > 0 && NULL == data) {
        UDS_LOGE(DBG_TAG, "size > 0 but data is null");
        return UDS_ERR_INVALID_ARG;
    }
    if (size > 0) {
...
UDSErr_t UDSUnpackRequestDownloadResponse(const UDSClient_t *client,
    ...
    if (sizeof(resp->maxNumberOfBlockLength) < maxNumberOfBlockLengthSize) {
        UDS_LOGI(DBG_TAG, "WARNING: sizeof(maxNumberOfBlockLength) > sizeof(size_t)");
        return UDS_FAIL;
    }
...
static inline UDSErr_t NegativeResponse(UDSReq_t *r, UDSErr_t nrc) {
    if (nrc < 0 || nrc > 0xFF) {
        UDS_LOGW(DBG_TAG, "Invalid negative response code: %d (0x%x)", nrc, nrc);
...
static UDSErr_t EmitEvent(UDSServer_t *srv, UDSEvent_t evt, void *data) {
    ...
    if (srv->fn) {
        ...
        UDS_LOGI(DBG_TAG, "Unhandled UDSEvent %d, srv.fn not installed!\n", evt);
        err = UDS_NRC_GeneralReject;
    }
    if (!UDSErrIsNRC(err)) {
        UDS_LOGW(DBG_TAG, "The returned error code %d (0x%x) is not a negative response code", err,
                 err);
    }
    return err;
}
...
static UDSErr_t Handle_0x19_ReadDTCInformation(UDSServer_t *srv, UDSReq_t *r) {
    ...
    case 0x56: /* reportDTCInformationByDTCReadinessGroupIdentifier */
        ...
        break;
    default:
        UDS_LOGW(DBG_TAG, "RDTCI subFunc 0x%02X is not supported.\n", type);
        return NegativeResponse(r, UDS_NRC_SubFunctionNotSupported);
    }

    return UDS_PositiveResponse;
respond_to_0x19_malformed_response:
    UDS_LOGE(DBG_TAG, "RDTCI subFunc 0x%02X is malformed. Length: %zu\n", type, r->send_len);
...
static UDSErr_t Handle_0x22_ReadDataByIdentifier(UDSServer_t *srv, UDSReq_t *r) {
    ...
    for (uint16_t did = 0; did < numDIDs; did++) {
        ...
        if (ret == UDS_PositiveResponse && send_len_before == r->send_len) {
            UDS_LOGE(DBG_TAG, "RDBI response positive but no data sent\n");
            return NegativeResponse(r, UDS_NRC_GeneralReject);
        }
...
static UDSErr_t Handle_0x23_ReadMemoryByAddress(UDSServer_t *srv, UDSReq_t *r) {
    ...
    if (r->send_len != UDS_0X23_RESP_BASE_LEN + length) {
        UDS_LOGE(DBG_TAG, "response positive but not all data sent: expected %zu, sent %zu",
        ...
        */
        if (subFunction == srv->securityLevel) {
            ...
            if (r->send_len <= UDS_0X27_RESP_BASE_LEN) { // no data was copied
                UDS_LOGE(DBG_TAG, "0x27: no seed data was copied");
                return NegativeResponse(r, UDS_NRC_GeneralReject);
            }
            return UDS_PositiveResponse;
        }
...
static UDSErr_t Handle_0x2C_DynamicDefineDataIdentifier(UDSServer_t *srv, UDSReq_t *r) {
    ...
    default:
        UDS_LOGW(DBG_TAG, "Unsupported DDDI subFunc 0x%02X\n", type);
        return NegativeResponse(r, UDS_NRC_SubFunctionNotSupported);
    }
}
...
static UDSErr_t Handle_0x34_RequestDownload(UDSServer_t *srv, UDSReq_t *r) {
    ...
    if (args.maxNumberOfBlockLength < 3) {
        UDS_LOGE(DBG_TAG, "maxNumberOfBlockLength too short");
        return NegativeResponse(r, UDS_NRC_GeneralReject);
    }
...
static UDSErr_t Handle_0x87_LinkControl(UDSServer_t *srv, UDSReq_t *r) {
    ...
    if (type == 0x03 && (r->recv_buf[1] & 0x80) == 0 &&
        ...
        UDS_LOGW(DBG_TAG, "0x87 LinkControl: Transitioning mode without suppressing response!");
    }
...
static UDSService getServiceForSID(uint8_t sid) {
    ...
    default:
        UDS_LOGI(DBG_TAG, "no handler for request SID %x", sid);
        return NULL;
    }
}
    ...
    if (srv->requestInProgress) {
        ...
        if (UDSTimeAfter(UDSMillis(), srv->p2_timer)) {
            ...
            if (ret < 0) {
                ...
                UDS_LOGE(DBG_TAG, "UDSTpSend failed with %zd\n", ret);
            }
        ...
        if (len < 0) {
            UDS_LOGE(DBG_TAG, "UDSTpRecv failed with %zd\n", r->recv_len);
            return;
        }
...
static ssize_t tp_send(UDSTp_t *hdl, uint8_t *buf, size_t len, UDSSDU_t *info) {
    ...
    case UDS_A_TA_TYPE_FUNCTIONAL:
        link = &tp->func_link;
        if (len > 7) {
            UDS_LOGI(DBG_TAG, "Cannot send more than 7 bytes via functional addressing\n");
            ret = -3;
            goto done;
        }
        break;
...
static ssize_t tp_recv(UDSTp_t *hdl, uint8_t *buf, size_t bufsize, UDSSDU_t *info) {
    ...
    if (ret == ISOTP_RET_OK) {
        UDS_LOGI(DBG_TAG, "phys link received %d bytes", out_size);
        if (NULL != info) {
        ...
        if (ret == ISOTP_RET_OK) {
            UDS_LOGI(DBG_TAG, "func link received %d bytes", out_size);
            if (NULL != info) {
            ...
            UDS_LOGE(DBG_TAG, "unhandled return code from func link %d\n", ret);
        }
    } else {
        UDS_LOGE(DBG_TAG, "unhandled return code from phys link %d\n", ret);
    }
    return out_size;
}
...
static int SetupSocketCAN(const char *ifname) {
    ...
    if (snprintf(ifr.ifr_name, sizeof(ifr.ifr_name), "%s", ifname) >= (int)sizeof(ifr.ifr_name)) {
        UDS_LOGE(DBG_TAG, "Interface name too long");
        close(sockfd);
        sockfd = -1;
        goto done;
    }
...
static ssize_t isotp_c_socketcan_tp_send(UDSTp_t *hdl, uint8_t *buf, size_t len, UDSSDU_t *info) {
    ...
    UDS_LOGD(DBG_TAG, "'%s' sends %ld bytes to 0x%03x (%s)", tp->tag, len, ta,
             ta_type == UDS_A_TA_TYPE_PHYSICAL ? "phys" : "func");
    UDS_LOG_SDU(DBG_TAG, buf, len, info);
    return ret;
}
...
static UDSTpStatus_t isotp_sock_tp_poll(UDSTp_t *hdl) {
    ...
    ret = poll(pfds, 2, 1);
    if (ret < 0) {
        UDS_LOGE(DBG_TAG, "poll failed: %d", ret);
        status |= UDS_TP_ERR;
    } else if (ret == 0) {
        ; // timeout, no events
    } else {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            if (pfd.revents & POLLERR) {
                ...
                if (!getsockopt(fds[i], SOL_SOCKET, SO_ERROR, &pending_err, &len) && pending_err) {
                    switch (pending_err) {
                    case ECOMM:
                        UDS_LOGE(DBG_TAG, "ECOMM: Communication error on send");
                        status |= UDS_TP_ERR;
                        break;
                    default:
                        UDS_LOGE(DBG_TAG, "Asynchronous socket error: %s (%d)",
                                 strerror(pending_err), pending_err);
                        status |= UDS_TP_ERR;
                        break;
                    }
                } else {
                    UDS_LOGE(DBG_TAG, "POLLERR was set, but no error returned via SO_ERROR?");
                }
            }
...
static ssize_t tp_recv_once(int fd, uint8_t *buf, size_t size) {
    ...
    if (ret < 0) {
        if (EAGAIN == errno || EWOULDBLOCK == errno) {
            ...
            UDS_LOGI(DBG_TAG, "read failed: %ld with errno: %d\n", ret, errno);
            if (EILSEQ == errno) {
                UDS_LOGI(DBG_TAG, "Perhaps I received multiple responses?");
            }
        }
    }
    return ret;
...
static ssize_t isotp_sock_tp_recv(UDSTp_t *hdl, uint8_t *buf, size_t bufsize, UDSSDU_t *info) {
    ...
    if (ret > 0) {
        ...
        UDS_LOGD(DBG_TAG, "'%s' received %ld bytes from 0x%03x (%s), ", impl->tag, ret, msg->A_TA,
                 msg->A_TA_Type == UDS_A_TA_TYPE_PHYSICAL ? "phys" : "func");
        UDS_LOG_SDU(DBG_TAG, impl->recv_buf, ret, msg);
    }
...
static ssize_t isotp_sock_tp_send(UDSTp_t *hdl, uint8_t *buf, size_t len, UDSSDU_t *info) {
    ...
    if (UDS_A_TA_TYPE_PHYSICAL == ta_type) {
        ...
        if (len > 7) {
            UDS_LOGI(DBG_TAG, "UDSTpIsoTpSock: functional request too large");
            return -1;
        }
        fd = impl->func_fd;
    } else {
    ...
    UDS_LOGD(DBG_TAG, "'%s' sends %ld bytes to 0x%03x (%s)", impl->tag, len, ta,
             ta_type == UDS_A_TA_TYPE_PHYSICAL ? "phys" : "func");
    UDS_LOG_SDU(DBG_TAG, buf, len, info);
...
static int LinuxSockBind(const char *if_name, uint32_t rxid, uint32_t txid, bool functional) {
    ...
    if (functional) {
        UDS_LOGI(DBG_TAG, "configuring fd: %d as functional", fd);
...
UDSErr_t UDSTpIsoTpSockInitServer(UDSTpIsoTpSock_t *tp, const char *ifname, uint32_t source_addr,
    ...
    if (tp->phys_fd < 0 || tp->func_fd < 0) {
        UDS_LOGI(DBG_TAG, "foo\n");
        (void)fflush(stdout);
        return UDS_FAIL;
    }
    const char *tag = "server";
    memmove(tp->tag, tag, strlen(tag));
    UDS_LOGI(DBG_TAG, "%s initialized phys link rx 0x%03x tx 0x%03x func link rx 0x%03x tx 0x%03x",
...
static void NetworkPoll(void) {
    for (unsigned i = 0; i < MsgCount; i++) {
        if (UDSTimeAfter(UDSMillis(), msgs[i].scheduled_tx_time)) {
                ...
                if (tp->sa_phys == msgs[i].info.A_TA || tp->sa_func == msgs[i].info.A_TA) {
                    ...
                    UDS_LOGD(__FILE__,
                             ...
                             msgs[i].info.A_TA_Type == UDS_A_TA_TYPE_PHYSICAL ? "PHYSICAL"
                                                                              : "FUNCTIONAL");
                    UDS_LOG_SDU(DBG_TAG, msgs[i].buf, msgs[i].len, &(msgs[i].info));
            ...
            if (!found) {
                UDS_LOGW(DBG_TAG, "TPMock: no matching receiver for message");
            }
...
static ssize_t mock_tp_send(struct UDSTp *hdl, uint8_t *buf, size_t len, UDSSDU_t *info) {
    ...
    if (MsgCount >= NUM_MSGS) {
        UDS_LOGW(DBG_TAG, "mock_tp_send: too many messages in the queue");
        return -1;
    }
    ...
    if (UDS_A_TA_TYPE_PHYSICAL == ta_type) {
        ...
        if (len > 7) {
            UDS_LOGW(DBG_TAG, "mock_tp_send: functional message too long: %ld", len);
            return -1;
        }
        m->info.A_TA = tp->ta_func;
        m->info.A_SA = tp->sa_func;
    } else {
        UDS_LOGW(DBG_TAG, "mock_tp_send: unknown TA type: %d", ta_type);
        return -1;
    }
    ...
    UDS_LOGD(DBG_TAG, "%s sends %ld bytes to TA=0x%03X (A_TA_Type=%s):", tp->name, len,
             m->info.A_TA, m->info.A_TA_Type == UDS_A_TA_TYPE_PHYSICAL ? "PHYSICAL" : "FUNCTIONAL");
    UDS_LOG_SDU(DBG_TAG, buf, len, &m->info);
...
static ssize_t mock_tp_recv(struct UDSTp *hdl, uint8_t *buf, size_t bufsize, UDSSDU_t *info) {
    ...
    if (bufsize < tp->recv_len) {
        UDS_LOGW(DBG_TAG, "mock_tp_recv: buffer too small: %ld < %ld", bufsize, tp->recv_len);
        return -1;
    }
...
static void ISOTPMockAttach(ISOTPMock_t *tp, ISOTPMockArgs_t *args) {
    ...
    tp->recv_len = 0;
    UDS_LOGV(DBG_TAG, "attached %s. TPCount: %d", tp->name, TPCount);
}
...
static void ISOTPMockDetach(ISOTPMock_t *tp) {
    ...
    for (unsigned i = 0; i < TPCount; i++) {
        if (TPs[i] == tp) {
            ...
            TPCount--;
            UDS_LOGV(DBG_TAG, "TPMock: detached %s. TPCount: %d", tp->name, TPCount);
            return;
        }
    }
    UDS_ASSERT(false);
...
UDSTp_t *ISOTPMockNew(const char *name, ISOTPMockArgs_t *args) {
    if (TPCount >= MAX_NUM_TP) {
        UDS_LOGI(DBG_TAG, "TPCount: %d, too many TPs\n", TPCount);
        return NULL;
    }
    ...
    if (name) {
        if (snprintf(tp->name, sizeof(tp->name), "%s", name) >= (int)sizeof(tp->name)) {
            UDS_LOGE(DBG_TAG, "Transport name too long, truncated");
        }
    } else {
...
void isotp_on_can_message(IsoTpLink* link, const uint8_t* data, uint8_t len) {
    ...
    switch (message.as.common.type) {
        ...
        case ISOTP_PCI_TYPE_FIRST_FRAME: {
            // rt_kprintf("[ISOTP] receive first frame\n");
            /* update protocol result */
            if (ISOTP_RECEIVE_STATUS_INPROGRESS == link->receive_status) {
                link->receive_protocol_result = ISOTP_PROTOCOL_RESULT_UNEXP_PDU;
            } else {
        ...
        case TSOTP_PCI_TYPE_CONSECUTIVE_FRAME: {
            //rt_kprintf("[ISOTP] receive consecutive frame\n");
