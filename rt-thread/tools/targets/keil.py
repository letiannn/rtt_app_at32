...
def MDK5Project(env, target, script):
    ...
    if os.path.isfile('template/template.uvprojx') is False:
        print ('Warning: The template project file [template/template.uvprojx] not found!')
        return

    template_tree = etree.parse('template/template.uvprojx')
    ...
    if os.path.isfile(project_uvopt):
        os.unlink(project_uvopt)
    # copy uvopt file
    if os.path.exists('template/template.uvoptx'):
        import shutil
        shutil.copy2('template/template.uvoptx', '{}.uvoptx'.format(os.path.splitext(target)[0]))
        # build with UV4.exe
